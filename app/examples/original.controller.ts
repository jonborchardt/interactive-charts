// module InteractiveChartsExampleApp {
    ...
    export interface IOriginalController {
    ...
    export interface IOriginalControllerScope extends ng.IScope {
        vm: IOriginalController;
    }

    export class OriginalController implements IOriginalController {
        scope: IOriginalControllerScope;
        ...
        // add enums for use in template
        public enums: { chartName: {}; chartGroupName: {} } = { chartName: ChartName, chartGroupName: ChartGroupName };
        ...
        constructor($httpBackend: ng.IHttpBackendService,
            ...
            // code for testing
            let startDate: moment.Moment = moment("2016 01 01", "YYYY MM DD").startOf("week").add(1, "day");
            ...
            let randSpend = d3.random.normal(4000, 1000);
            let randSpendDif = d3.random.normal(0, 1000);
            let randImps = d3.random.normal(4500000, 1125000);
            let randImpsDif = d3.random.normal(0, 1125000);
    ...
    // InteractiveChartsExampleApp.Modules.App.controller("originalController", [
    //     "$httpBackend", "$interval", "$q", "$rootScope", "$scope", "$timeout",
    //     ($httpBackend: ng.IHttpBackendService, $interval: ng.IIntervalService, $q: ng.IQService, $rootScope: ng.IRootScopeService, $scope: IOriginalControllerScope,
    //         $timeout: ng.ITimeoutService): InteractiveChartsExampleApp.TvReportCtrl =>
    //         new InteractiveChartsExampleApp.TvReportCtrl($httpBackend, $interval, $q, $rootScope, $scope, $timeout)
    // ]);
// }